...
class YearlyExamRecordReport < BaseReport
  ...
  def build(
    ...
    steps.each_with_index do |step, index|
      ...
      @daily_notes = form.daily_notes(step)
      @info_students = form.filter_unique_students(step)
      @complementary_exams = form.complementary_exams(step)
      @school_term_recoveries = form.school_term_recoveries(step)
      @recovery_lowest_notes = form.recovery_lowest_notes?(step)
      @lowest_notes = form.lowest_notes(step)

      header
      content
      footer
      start_new_page unless index == steps.size - 1
    end
...
content: "#{entity_name}\n#{organ_name}\n#{@classroom.unity.name}", size: 12, leading: 1.5, align: :center, valign: :center, rowspan: 4, padding: [
      6, 2, 8, 2
])
